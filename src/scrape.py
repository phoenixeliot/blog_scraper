...
def remove_blacklisted_selectors(body_soup):
    if config["blacklist_selector"]:
        for tag in body_soup.select(config["blacklist_selector"]):
            print(
                f"Removing blacklisted item by selector ({config['blacklist_selector']}): \"{tag.text.strip()}\""
            )
            tag.decompose()
...
if config[
    "rewrite_post"
]:  # TODO: Try to make this not run twice? Though running it twice might be necessary for some current hacks...
    for post in posts:
    ...
    while len(extra_page_urls) > 0:
        ...
        for scrape_result in scraped_extra_pages:
            ...
            try:
                ...
                final_url_to_id[scrape_result["final_url"]] = "extra" + str(extra_count)
                extra_count += 1
            except Exception as e:
...
if config["scrape_images"]:
    ...
    def multi_scrape_image(image_scraper, src):
        try:
            ...
            absolute_image_path = os.path.join(
                os.path.dirname(__file__), "..", "books", image_path
            )
    ...
    for scraped_image in scraped_images:
        ...
        if (
            scraped_image["response"] is not None
        ):  # It's not cached # TODO: Distinguish from Selenium requests here that don't have a response object
            if scraped_image["response"].status_code != 200:
                print(
                    f"WARNING: Failed to scrape image: {scraped_image['src']}, with status code {scraped_image['response'].status_code}"
                )
        ...
        for image_tag in images_by_src[scraped_image["src"]]:
            ...
            if (
                False and content_type == "image/svg+xml"
            ):  # TODO: Verify this still works after the below changes; or, save it as a file too.
                ...
                image_tag[
                    "style"
                ] = "max-height: 700px; max-width: 500px; object-fit: contain; margin: 0; padding: 0;"
                if int(image_tag.get("height", 0)) > 200:
                    soup = BeautifulSoup()
                    wrap_div = soup.new_tag("div")
                    wrap_div[
                        "style"
                    ] = "page-break-inside: avoid; page-break-before: always; margin: 0; padding: 0;"
...
if config["toc_keep_original_formatting"]:
    ...
    if config["crawl_mode"] == "toc":
        toc_result_html = '<h1 id="toc">Table of Contents</h1>' + "<br>".join(
            map(lambda link: str(link["tag"]), toc_links)
        )
    elif config["crawl_mode"] == "incremental":

        def toc_link_from_post(post):
            return f"<a href=\"#{final_url_to_id[post['final_url']]}\">{post['title_soup'].text}</a>"

        toc_result_html = '<h1 id="toc">Table of Contents</h1>' + "<br>".join(
            map(toc_link_from_post, posts)
        )
