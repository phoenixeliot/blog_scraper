...
async function main() {
  ...
  const customReplacements = [
    ...
    ["CUSTOM_PUNCTUATION", " , ", ", "],
    ["CUSTOM_PUNCTUATION", ",(", ", ("],
    ["CUSTOM_PUNCTUATION", /\. *\. *\.? */g, "... "], // 2 or 3 periods with or without spaces
    ["CUSTOM_PUNCTUATION", " . ", ". "],
    ["CUSTOM_PUNCTUATION", " - ", " — "],
    ["CUSTOM_PUNCTUATION", " -- ", " — "],
    ["CUSTOM_PUNCTUATION", " --- ", " — "],
  ...
  ] as [string, RegExp | string, string][];
    ...
    for (const [ruleId, pattern, replacement] of customReplacements) {
      if (pattern.constructor.name === "RegExp") {
        const regexMatches = Array.from(
          text.matchAll(pattern, { global: true }),
        ) as any[];
        let continueFrom = 0;
        if (regexMatches.length > 0) {
          for (const regexMatch of regexMatches) {
            ...
            const caseCheckedReplacement = replaceWithCase(
              original,
              replacement,
            );
        ...
        while (text.slice(searchFrom).indexOf(pattern) > -1) {
          ...
          matches.push({
            ...
            length: (pattern as string).length,
            replacements: [replacement],
          });
          searchFrom += index + (pattern as string).length;
        }
      }
    }
    return matches;
...
*/
  ...
  // // Clear out the database before populating it
  // await prisma.ruleInstance.deleteMany()
  // await prisma.paragraph.deleteMany()

  const startingParagraph = currentParagraph;
  for (let _index in paragraphs.slice(
    startingParagraph,
    // currentParagraph + 400,
  )) {
    const index = Number(_index) + startingParagraph;
    const paragraph = paragraphs[index];
    if (stop) break;
    currentParagraph += 1;
    // if (paragraph.textContent !== paragraph.innerHTML) {
    //   // We can't handle paragraphs with tags inside them yet
    //   continue;
    // }
    ...
    const { id: paragraphId } = await prisma.paragraph.upsert({
      where: { filename_index: { filename, index: currentParagraph } },
      create: { filename, index, originalText: text },
      update: {},
    });
    await check(text, "en-US")
      .then(async (res) => {
        ...
        for (const match of res.matches) {
          ...
          await prisma.ruleInstance.upsert({
            where: {
              paragraphId_ruleId: { paragraphId, ruleId },
            },
            create: { paragraphId, ruleId },
            update: {},
          });
